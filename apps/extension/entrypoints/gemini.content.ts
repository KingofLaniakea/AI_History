...
const GEMINI_CAPTURE_DEBUG_VERSION = "2026-02-23-r9-real-attachments";
...
export default defineContentScript({
  ...
  main() {
    ...
    if (globalWindow[GEMINI_CAPTURE_BIND_KEY]) {
      return;
    }
    globalWindow[GEMINI_CAPTURE_BIND_KEY] = true;
    ...
    chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
      ...
      void (async () => {
        ...
        await warmupSourceLazyResources("gemini");
        ...
        try {
          finalizedTurns = await materializeAttachmentsOrThrow("gemini", withDrive, {
          ...
          console.warn("[AI_HISTORY] gemini attachment stage failed, fallback to content-only import", error);
